...
import os
import re
import socket  # noqa: F401 (kept for `task.socket` back-compat access, e.g. in tests)
import time
from os import path
...
# Cross-post publishing (scheduling, worker pool, startup recovery) lives in
# its own module; re-exported here so ``task.<name>`` keeps working for
# every caller (WebUI, API, tests) exactly as before the split.
from app.services.task_cross_post import (  # noqa: F401
    _ACTIVE_CROSS_POST_STATES,
    _CROSS_POST_SOCIAL_PLATFORMS,
    _CROSS_POST_STATE_RETRY_DELAY_SECONDS,
    _CROSS_POST_STATE_WRITE_ATTEMPTS,
    _INTERRUPTED_CROSS_POST_ERROR,
    _UPLOAD_POST_RESULT_PLATFORM,
    _cross_post_executor,
    _cross_post_max_pending_tasks,
    _cross_post_futures,
    _cross_post_process_owner,
    _cross_post_registry_lock,
    _cross_post_slots,
    _ensure_cross_post_terminal_state,
    _finalize_cross_post_future,
    _is_cross_post_active_in_process,
    _is_cross_post_owner_alive,
    _is_windows_process_alive,
    _normalize_publish_result,
    _patch_cross_post_state,
    _record_cross_post_failure,
    _register_cross_post_future,
    _resolve_publish_targets,
    _run_cross_post,
