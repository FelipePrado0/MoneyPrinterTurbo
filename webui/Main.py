...
def _open_task_path(task_path):
    ...
    if _is_headless_server():
        ...
        # 用正斜杠拼接，避免 relpath 在部分平台返回反斜杠导致提示路径不一致。
...
def _render_video_settings(panel, params):
    ...
    saved_video_source_name = str(config.app.get("video_source", "pexels") or "pexels")
    video_source_label = video_source_labels.get(
...
def _render_subtitle_settings(panel, params):
    ...
    if subtitle_currently_enabled:
        ...
        subtitle_position_labels = {
            ...
            "center": tr("Center"),
            "bottom": tr("Bottom"),
            "two_thirds_bottom": tr("2/3 from Bottom"),
            "custom": tr("Custom"),
        }
